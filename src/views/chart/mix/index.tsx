      ...
      transform: [
        {
          type: 'custom',
          callback: (data: any[]) => {
            const sum = data.reduce((r: any, curr: { value: any; }) => r + curr.value, 0);
            return data
              .map((d: { value: number; }) => ({
                ...d,
                percentage: d.value / sum,
              }))
              .reduce((r: any[], curr: { percentage: any; }) => {
    ...
    children: [
      {
        ...
        style: { fill: (d: { percentage: number; }) => (d.percentage < 0.1 ? '#E24B26' : '#78B3F0') },
